...
import type { AppraisalReviewResponse, EmployeeAppraisalFormResponse } from '../../types/appraisal';
...
const AppraisalEmployeeReviewsPage = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : 'HR reviewed appraisal records could not be loaded.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const employeeIdOptions = useMemo(() => {
    ...
    const options = new Map<number, string>();
    forms.forEach((form) => {
      if (selectedDepartmentId !== null && form.departmentId !== selectedDepartmentId) return;
  ...
  useEffect(() => {
    ...
    const stillAvailable = employeeIdOptions.some(([employeeId]) => String(employeeId) === employeeIdFilter);
  ...
  const filteredForms = useMemo(() => {
    ...
    const selectedDepartmentId = departmentFilter ? Number(departmentFilter) : null;
    const selectedEmployeeId = employeeIdFilter ? Number(employeeIdFilter) : null;

    return forms.filter((form) => {
      const departmentMatches = selectedDepartmentId === null || form.departmentId === selectedDepartmentId;
      const employeeMatches = selectedEmployeeId === null || form.employeeId === selectedEmployeeId;
      ...
      <div className="appraisal-dashboard-hero appraisal-review-hero">
        ...
        <div className="appraisal-dashboard-hero-content">
          <div>
            ...
            <p>View employee appraisal forms that HR has reviewed and submitted to employees.</p>
          </div>
          <div className="appraisal-hero-stat-stack">
            <div className="appraisal-hero-stat-card"><strong>{stats.reviewed}</strong><span>Reviewed Forms</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="appraisal-card">
        ...
        <div className="appraisal-filter-bar">
          ...
          <label className="appraisal-filter-field">
            ...
            <select value={employeeIdFilter} onChange={(event) => setEmployeeIdFilter(event.target.value)}>
              <option value="">All Employee IDs</option>
              {employeeIdOptions.map(([employeeId, label]) => (
                <option key={employeeId} value={employeeId}>{label}</option>
              ))}
            </select>
          </label>
          <div className="appraisal-filter-actions">
            <button className="appraisal-button ghost" type="button" onClick={clearEmployeeReviewFilters}>Clear Filters</button>
          ...
          <table className="appraisal-template-table appraisal-cycle-record-table appraisal-modern-record-table">
            ...
            <tbody>
              {filteredForms.length === 0 && !loading ? (
                <tr>
                  <td colSpan={10}><div className="appraisal-empty">{forms.length === 0 ? 'No HR reviewed appraisal records found.' : 'No employee review records match the selected filters.'}</div></td>
                </tr>
                ...
                return (
                  ...
                  >
                    ...
                    <td>{hrReview?.reviewerEmployeeId || form.hrCheckedByEmployeeId || '-'}</td>
                    <td>{form.departmentName}</td>
                    <td>{form.cycleType || '-'}</td>
                    <td>{form.cycleYear || '-'}</td>
                    <td>{formatDate(form.cycleEndDate)}</td>
                    <td>{formatDateTime(hrReview?.submittedAt || form.hrApprovedAt)}</td>
        ...
        <div className="appraisal-modal-backdrop" onClick={closeFormModal}>
          ...
          >
            ...
            <div className="appraisal-modal-body template-form-modal-body">
              <AppraisalFormView form={selected} mode="readonly" busy={loading} />
            </div>
...
const statusClass = (status?: string | null) => {
  ...
  if (normalized === 'RETURNED' || normalized === 'REJECTED') return 'appraisal-status red';
