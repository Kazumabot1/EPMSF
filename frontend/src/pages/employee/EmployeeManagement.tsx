...
const EmployeeManagement = () => {
  ...
  const filtered = useMemo(() => {
    let list = employees.filter((emp) => {
      ...
      const matchSearch = !q || displayName.toLowerCase().includes(q) || (emp.staffNrc || '').toLowerCase().includes(q);
      ...
      <div className="epms-emp-main relative z-10 mx-auto max-w-7xl px-4 pb-10 pt-6 sm:px-5">
        <header className="epms-emp-hero">
          ...
          <p className="epms-emp-lead">
            View and manage employee master data. Login accounts and Excel/CSV import are unchanged; use{' '}
            <Link to="/hr/employee/import" className="text-indigo-600 underline decoration-indigo-200 underline-offset-2">
              Import Employees
            </Link>{' '}
            for bulk user onboarding.
          </p>
          <div className="epms-emp-hero-actions flex flex-wrap gap-2">
            <button type="button" className="epms-emp-btn epms-emp-btn--primary" onClick={openCreate}>
              <i className="bi bi-person-plus" aria-hidden />
              Add employee
            </button>
            <Link to="/hr/employee/import" className="epms-emp-btn epms-emp-btn--import no-underline">
        ...
        <div className="epms-emp-toolbar">
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-end">
            <label className="flex min-w-[140px] flex-col text-sm text-slate-600">
              <span className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-500">Gender</span>
              <select
        ...
        {loading ? (
          ...
          <>
                      ...
                      <tr key={emp.id}>
                        <td>
                          <div className="epms-emp-name-cell">
                            <span
                              className={`epms-emp-avatar ${!active ? 'epms-emp-avatar--inactive' : ''}`}
                              aria-hidden
                            >
                              {initials(emp)}
                            </span>
                        ...
                        <td className="max-w-[160px] truncate" title={emp.positionTitle || ''}>
                          {emp.positionTitle
                            ? `${emp.positionTitle}${emp.positionLevelCode ? ` (${emp.positionLevelCode})` : ''}`
                            : '—'}
                        </td>
                        ...
                        <td>{emp.gender || '—'}</td>
                        <td className="max-w-[180px] truncate" title={emp.currentDepartment || emp.parentDepartment || ''}>
                          {emp.currentDepartment || emp.parentDepartment || '—'}
                        </td>
                        <td>
                          <span className={`epms-emp-badge ${active ? 'epms-emp-badge--on' : 'epms-emp-badge--off'}`}>
            ...
            <div className="epms-emp-pager hidden md:flex">
              <span>
                Page {currentPage} of {totalPages} · {filtered.length} row{filtered.length === 1 ? '' : 's'}
              </span>
                ...
                <div key={`m-${emp.id}`} className="epms-emp-card">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex min-w-0 items-center gap-2">
                      <span
                        className={`epms-emp-avatar shrink-0 ${!active ? 'epms-emp-avatar--inactive' : ''}`}
                        aria-hidden
                      >
                        {initials(emp)}
                      </span>
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-sm text-slate-400">{emp.phoneNumber || '—'}</p>
                      </div>
                    </div>
                    <span className={`epms-emp-badge shrink-0 ${active ? 'epms-emp-badge--on' : 'epms-emp-badge--off'}`}>
                      {active ? 'Active' : 'Inactive'}
                    </span>
                  </div>
                  <div className="mt-3 flex justify-end gap-1 border-t border-slate-100 pt-2">
                    <button type="button" className="epms-emp-icon-btn" onClick={() => openView(emp)} title="View">
                      <i className="bi bi-eye" />
                    </button>
                    <button type="button" className="epms-emp-icon-btn" onClick={() => openEdit(emp)} title="Edit">
