...
const KpiTemplateEditorPage = () => {
  ...
  const [existingTemplate, setExistingTemplate] =
    useState<ExistingKpiForPosition | null>(null);
  ...
  const [savingAction, setSavingAction] = useState<'draft' | 'use-in-cycle' | null>(
    null,
  );
  const [pageError, setPageError] = useState('');

  const applyLookups = useCallback(
    (lookups: Awaited<ReturnType<typeof loadKpiTemplateEditorLookups>>) => {
  ...
  useEffect(() => {
    const bootstrap = async () => {
      try {
        ...
        const excludeFormId =
          isEdit && !Number.isNaN(templateId) ? templateId : undefined;
        ...
        const message = toApiRequestError(err, 'Failed to load form.').message;
  ...
  const validate = (submitStatus: KpiFormStatus): string | null => {
    ...
    /*
     * Draft is intentionally lenient.
     * It only requires title + position.
     */
    ...
    if (rows.length === 0) {
      return 'Add at least one KPI row.';
    }
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      if (!hasCatalog && !hasLabel) {
        return `Row ${index + 1}: choose a catalog KPI or enter a KPI label.`;
      }

      if (
        row.kpiCategoryId === null ||
        row.kpiUnitId === null ||
        row.target === null ||
        row.weight === null
      ) {
        return `Row ${index + 1}: category, unit, target, and weight are required.`;
      }
    }

    if (
      (submitStatus === 'ACTIVE' || submitStatus === 'FINALIZED') &&
      totalWeight !== 100
    ) {
      return 'Total weight must equal 100% for ACTIVE / Use Form.';
    }
  ...
  const saveTemplate = async (
    ...
    const validationMessage = validate(submitStatus);
    ...
    if (positionId == null) {
      showValidation('Select a position.');
      return null;
    }
    ...
    try {
      ...
      if (action === 'use-in-cycle') {
        toast.success(
          isEdit ? 'KPI template activated.' : 'KPI template created and activated.',
        );
      ...
      if (!handled) {
        const message = toApiRequestError(
          err,
          'Could not save the KPI template.',
        ).message;
        setPageError(message);
        toast.error(message);
      }
          ...
          <section className="kpi-tpl-card overflow-hidden p-0">
              ...
              <div className="mb-8 flex flex-wrap items-center gap-4 border-b border-gray-100 pb-6">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-sm text-gray-500">
                    Template title and position assignment
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-x-8 gap-y-6 md:grid-cols-2">
                ...
                <label className="flex flex-col gap-2">
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="">
                        {positions.length === 0 ? 'No positions in system' : 'Select position…'}
                      </option>

                      {positionOptions.map((option) => (
                        <option
                          key={option.position.id}
                          value={option.position.id}
                          disabled={false}
                        >
                          {option.label}
                        </option>
                  ...
                  {positions.length > 0 && (
                    <p className="text-sm text-gray-500">
                      {isEdit
                        ...
                        : `${availablePositionCount} option${availablePositionCount === 1 ? '' : 's'} available. Existing forms can still be updated by selecting their position.`}
                    </p>
                  )}
                  ...
                  {!isEdit && existingTemplate && (
                    <KpiPositionExistingAlert
                      ...
                      onEdit={() =>
                        navigate(`/hr/kpi-template/${existingTemplate.templateId}/edit`)
                      }
                      onView={() =>
                        navigate(`/hr/kpi-template/${existingTemplate.templateId}`)
                      }
                    />
                  )}
                </label>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 max-w-xl text-sm text-gray-500">
                    Violet-tinted columns are filled later by PM: actual, score, and weighted score.
                  </p>
                </div>
              </div>
            ...
            <div className="p-4 sm:p-6">
              <KpiTemplateRowsTable
                ...
                onAddRow={() => {
                  setPageError('');
                  setRows((previous) => [...previous, newKpiTemplateRow()]);
                }}
                onRemoveRow={(rowId) =>
                  setRows((previous) =>
                    previous.length > 1
