...
import DepartmentKpiCycleViewModal from '../../../components/hr/department-kpi/DepartmentKpiCycleViewModal';
...
const hasGracePeriodEnded = (cycle: DepartmentKpiCycle) => {
...
const getCycleEditDisabledReason = (cycle: DepartmentKpiCycle) => {
...
const DepartmentKpiCycleListPage = () => {
  ...
  const load = async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to load Department KPI cycles.';
      setError(message);
      toast.error(message);
    } finally {
      setLoading(false);
  ...
  const handleToggleActive = async (cycle: DepartmentKpiCycle) => {
    ...
    try {
      ...
      toast.success(nextActive ? 'Department KPI cycle activated.' : 'Department KPI cycle deactivated.');
      await load();
    } catch (err) {
  ...
  const submitEarlyCloseRequest = async () => {
    ...
    try {
      ...
      await departmentKpiCycleService.updateStatus(closeCycle.id, {
        active: false,
          ...
          <div className="relative flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex max-w-2xl gap-5">
              ...
              <div className="min-w-0 pt-0.5">
                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-violet-700/90">Department KPI</p>
                <h1 className="mt-2 text-2xl font-bold tracking-tight text-gray-900 sm:text-3xl">Department KPI cycles</h1>
                <p className="mt-3 text-sm leading-relaxed text-gray-600">
                  Group department KPI templates into scoring windows and manage early-close approval requests.
                </p>
              </div>
            </div>
            <div className="flex shrink-0 flex-wrap items-center gap-3 lg:justify-end">
              ...
              <Link to="/hr/department-kpi-cycle/new" className="kpi-tpl-btn-primary no-underline">
        ...
        <section className="mt-10">
          ...
          {!loading && !error && sorted.length === 0 && (
            <div className="kpi-tpl-card px-6 py-16 text-center">
              <h2 className="text-xl font-bold text-gray-900">No Department KPI cycles yet</h2>
              <p className="mx-auto mt-3 max-w-md text-sm text-gray-600">
                Create a cycle to schedule Department KPI templates across a defined scoring window.
              </p>
              <Link to="/hr/department-kpi-cycle/new" className="kpi-tpl-btn-primary mt-8 inline-flex no-underline">
                Create cycle
              </Link>
            </div>
          )}
                ...
                <table className="min-w-[1100px] w-full border-collapse text-left text-sm">
                  <thead className="kpi-tpl-thead">
                    <tr className="text-[11px] font-bold uppercase tracking-wider text-gray-500">
                      ...
                      <th className="px-4 py-4">Selected Templates</th>
                  ...
                  <tbody className="divide-y divide-gray-100 bg-white">
                    {sorted.map((cycle, index) => {
                      ...
                      const templateNames = cycle.templates.map((template) => template.title).join(', ');
                      return (
                        <tr
                          key={cycle.id}
                        ...
                        >
                          ...
                          <td className="px-4 py-4 text-gray-700">
                            <p className="max-w-xs truncate" title={templateNames}>
                              {templateNames || '-'}
                            </p>
                          </td>
          ...
          <div className="kpi-tpl-reason-modal">
            ...
            <div className="kpi-tpl-modal-body space-y-4">
              <p className="text-sm leading-6 text-gray-600">
                ...
                Add a reason and grace period for in-progress scoring before sending this to CEO.
              </p>
